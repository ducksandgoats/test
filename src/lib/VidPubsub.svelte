<script>
  ...
  //     console.log(await (await fetch('pubsub://testing', {method: 'POST', body: JSON.stringify({proc: 'start', vid, id, mime: mediaRecorder.mimeType})})).text())
  //   })
  //   mediaRecorder.addEventListener('resume', async () => {
  //     console.log(await (await fetch('pubsub://testing', {method: 'POST', body: JSON.stringify({proc: 'resume', vid, id, mime: mediaRecorder.mimeType})})).text())
  //   })
  //   mediaRecorder.addEventListener('pause', async () => {
  //     console.log(await (await fetch('pubsub://testing', {method: 'POST', body: JSON.stringify({proc: 'pause', vid, id, mime: mediaRecorder.mimeType})})).text())
  //   })
  //   mediaRecorder.addEventListener('stop', async () => {
  //     console.log(await (await fetch('pubsub://testing', {method: 'POST', body: JSON.stringify({proc: 'stop', vid, id, mime: mediaRecorder.mimeType})})).text())
  //   })
	// 	mediaRecorder.addEventListener('dataavailable', async (ev) => {
	// 		document.getElementById('own').src = window.URL.createObjectURL(ev.data);
  //     console.log(await (await fetch('pubsub://testing', {method: 'POST', body: JSON.stringify({proc: 'data', vid, id, mime: mediaRecorder.mimeType, data: await ev.data.text()})})).text())
  //   })
	// })
