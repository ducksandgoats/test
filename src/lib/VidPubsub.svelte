<script>
  ...
  func().then(console.log).catch(console.error)
    ...
    mediaRecorder.addEventListener('start', async () => {
      console.log(await (await fetch('pubsub://testing', {method: 'POST', body: JSON.stringify({proc: 'start', id})})).text())
    })
    mediaRecorder.addEventListener('resume', async () => {
      console.log(await (await fetch('pubsub://testing', {method: 'POST', body: JSON.stringify({proc: 'resume', id})})).text())
    })
    mediaRecorder.addEventListener('pause', async () => {
      console.log(await (await fetch('pubsub://testing', {method: 'POST', body: JSON.stringify({proc: 'pause', id})})).text())
    })
    mediaRecorder.addEventListener('stop', async () => {
      console.log(await (await fetch('pubsub://testing', {method: 'POST', body: JSON.stringify({proc: 'stop', id})})).text())
    })
      ...
      console.log(await (await fetch('pubsub://testing', {method: 'POST', body: JSON.stringify({proc: 'data', id, data: await ev.data.text()})})).text())
    })
	})
